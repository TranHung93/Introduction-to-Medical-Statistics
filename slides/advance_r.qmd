...
- Into the `tidyverse`
- Data ingestion
- Pivot `tibble`s into long and wide formats
- How to handle `NA`s
- Select columns

**Afternoon**:

- Transforming `tibble`s
  - Column manipulation (mutate, rename, if else)
  - Type assignment (as.*)
- Joining `tibble`s
...
- For-loops
- Piping in R
- Summarising `tibble`s (push to day 5)
...
- Make your decision:
  ...
  - Data imputation, i.e. replace the `NA`s with something that makes sense like mean, mode, median, interpolation,...
  - Something else?
...
- You can further specify the column to drop `NA`s from
